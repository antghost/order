        ...
        <div class="row">
            ...
            <div class="col-md-9 col-md-offset-0">
                ...
                <div id="div_table" class="col-md-12" style="background-color: white; margin-top: 1px">
                    ...
                    <form id="form_menu" action="{{ url('staff/menu/mass_today') }}" method="post">
                        {{ csrf_field() }}
                    ...
                    <input type="button" class="btn btn-info" id="mass_add_menu" value="添加今日菜单" disabled="true">
                    <input type="button" class="btn btn-warning" id="mass_cancel_menu" value="取消今日菜单" disabled="true">
...
@section('script')
    ...
    <script>
        ...
        $("#div_table input:checkbox").click(function () {
            ...
            $("#mass_add_menu").prop('disabled', count == 0);
            $("#mass_cancel_menu").prop('disabled', count == 0);
        });
        ...
        $('#div_table').on('click', 'button[name="menu_today"]', function () {
            var id = $(this).val();
            $.post(url+ "/today/"+ id, function (data) {
        ...
        //批量添加今日菜单
        $('#mass_add_menu').click(function () {
            $('#form_menu input[name="type"]').val('1');
            $('#form_menu').submit();
            $(this).prop('disabled', true);
            $("#mass_cancel_menu").prop('disabled',true)
        })
        //批量取消今日菜单
        $('#mass_cancel_menu').click(function () {
            $('#form_menu input[name="type"]').val('0');
            $('#form_menu').submit();
            $(this).prop('disabled', true);
            $("#mass_add_menu").prop('disabled',true)
        })
        ...
        //编辑菜单
        $('#div_table').on('click', 'button[name="menu_edit"]', function () {
            var _href = url+ "/" + $(this).val()+ "/edit";
            layer.open({
                type: 2,
                title: '编辑菜式',
                area: ['400px', '500px'], //宽\高
